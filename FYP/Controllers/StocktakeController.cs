...
namespace FYP.Controller
{
    public class StocktakeController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        [Authorize(Roles = "Admin , Store Supervisor")]
        [HttpPost]
        public IActionResult AddStocktake(IFormFile postedFile, String storage_loocation)
        {
            if (postedFile != null)
            {
                try
                {
                    ...
                    var user = DBUtl.GetList<Users>("SELECT * FROM Users WHERE nric = '" + User.Identity.Name + " ' ");
                    var currequip = DBUtl.GetList<Equipment>("SELECT * FROM Equipment WHERE Storage_location = '" + storage_loocation + "'");
                    var curracess = DBUtl.GetList<Equipment_Accessories>("Select * From Equipment_accessories WHERE Storage_location ='" + storage_loocation + "'");
                    ...
                    foreach (var a in equipment)
                    {
                        ...
                        {
                            if (a.Storage_location.Equals(storage_loocation))
                            {
                    ...
                    {
                        if (c.Storage_location.Equals(storage_loocation))
                        {
                    ...
                    int curr_equip_avail = DBUtl.GetList<Equipment>("SELECT * FROM Equipment WHERE Storage_location = '" + storage_loocation + "' AND Status = 'Available'").Count;
                    ...
                    int resu = DBUtl.ExecSQL(insert, user[0].User_id, stock_equip, stock_access, DateTime.Now, archive, diff_equip, diff_access, storage_loocation);
                    ...
                    {
                        if (e.Storage_location.Equals(storage_loocation))
                        {
                    ...
                    {
                        if (f.Storage_location.Equals(storage_loocation))
                        {
