    ...
    public class ExerciseController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        public IActionResult Create(Exercise e)
        {
            ...
            else
            {
                ...
                string insert =
                    @"INSERT INTO Exercise(nric, Package_id, company, unit, description, start_date, end_date, archive, status)
                      Values ('{0}', '{1}', '{2}', '{3}', '{4}', '{5:yyyy-MM-dd}', '{6:yyyy-MM-dd}', '{7}', '{8}')";
                ...
                bool archived = false;
                int res = DBUtl.ExecSQL(insert, nricFinal, e.Package_id, e.company, e.unit, e.description, e.start_date, e.end_date, archived, "Pending");
